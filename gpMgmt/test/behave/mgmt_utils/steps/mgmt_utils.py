...
@given('the user waits until recovery_progress.file is created in {logdir}')
@when('the user waits until recovery_progress.file is created in {logdir}')
@then('the user waits until recovery_progress.file is created in {logdir}')
def impl(context, logdir):
    attempt = 0
    num_retries = 6000
    log_dir = _get_gpAdminLogs_directory() if logdir == 'gpAdminLogs' else logdir
