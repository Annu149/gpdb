...
#      | \S+     | [0-9]+ | incremental    | [0-9]+                 | [0-9]+             | [0-9]+\%             |
  ...
  @demo_cluster
  @concourse_cluster
  Scenario: gprecoverseg errors out if pg_basebackup already running for one of the failed segments
    Given the database is running
    And all the segments are running
    And the segments are synchronized
    And all files in gpAdminLogs directory are deleted on all hosts in the cluster
  ...
  @demo_cluster
  @concourse_cluster
  Scenario: gprecoverseg errors out if pg_basebackup already running for some of the failed segments
    Given the database is running
    And all the segments are running
    And the segments are synchronized
    And all files in gpAdminLogs directory are deleted on all hosts in the cluster
  ...
  @demo_cluster
  #@concourse_cluster
  Scenario: gprecoverseg -aF errors out if pg_basebackup already running for all of the failed segments
    Given the database is running
    And all the segments are running
    And the segments are synchronized
    And all files in gpAdminLogs directory are deleted on all hosts in the cluster
  ...
  @demo_cluster
  @concourse_cluster
  Scenario: gprecoverseg -i errors out if pg_basebackup already running for all failed segments
    Given the database is running
    And all the segments are running
    And the segments are synchronized
    And all files in gpAdminLogs directory are deleted on all hosts in the cluster
