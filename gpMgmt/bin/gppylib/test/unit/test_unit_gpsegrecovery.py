...
class DifferentialRecoveryTestCase(GpTestCase):
    def setUp(self):
        # TODO should we mock the set_recovery_cmd_results decorator and not worry about
        # testing the command results in this test class
        self.maxDiff = None
    ...
    def tearDown(self):
        super(DifferentialRecoveryTestCase, self).tearDown()
    ...
    @patch('gppylib.commands.unix.Rsync.__init__', return_value=None)
    @patch('gppylib.commands.unix.Rsync.run')
    def test_rsync_run_passes(self, run, init, mock1,mock2,mock3, mock4):
    ...
    def test_basebackup_run_passes(self, run, init, mock1, mock2,mock3, mock4, mock5):
        self.mock_pgbasebackup_init = init
        self.mock_pgbasebackup_run = run
        self.diff_recovery_cmd.run()
    ...
    def test_pg_start_backup_success(self, mock1, mock2):
        ...
        self.assertEqual([call('Successfully started backup for host sdw1, port 40000')],
    ...
    def test_pg_stop_backup_success(self, mock1, mock2):
        self.diff_recovery_cmd.pg_stop_backup()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Successfully stopped backup for host sdw1, port 40000')],
                         self.mock_logger.debug.call_args_list)

    @patch('gppylib.db.dbconn.connect', autospec=True)
    @patch('gppylib.db.dbconn.query', side_effect = Exception())
    @patch('gpsegrecovery.DifferentialRecovery.pg_stop_backup', return_value=Mock())
    def test_cleanup_backup_progress_exception(self, pg_stop_mock, mock1, mock2):
        self.diff_recovery_cmd.error_type = RecoveryErrorType.SYNC_ERROR
        with self.assertRaises(Exception) as ex:
    ...
    @patch('gpsegrecovery.DifferentialRecovery.pg_stop_backup', return_value=Mock())
    def test_cleanup_backup_progress_false(self, pg_stop_mock, mock1, mock2):
        self.diff_recovery_cmd.error_type = RecoveryErrorType.SYNC_ERROR
        self.diff_recovery_cmd.cleanup()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual(0, pg_stop_mock.call_count)
    ...
    def test_cleanup_backup_progress_true(self, mock1, mock2):
        self.diff_recovery_cmd.error_type = RecoveryErrorType.SYNC_ERROR
        self.diff_recovery_cmd.cleanup()
        self.assertEqual(2, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed'),
                          call('Successfully stopped backup for host sdw1, port 40000')],
                         self.mock_logger.debug.call_args_list)

    @patch('gppylib.commands.unix.Rsync.__init__', return_value=None)
    @patch('gppylib.commands.unix.Rsync.run')
    def test_sync_pg_data_success(self,run, init):
        self.diff_recovery_cmd.sync_pg_data()
        self.assertEqual(1, init.call_count)
        self.assertEqual(1, run.call_count)
        self.assertEqual(call(validateAfter=True), run.call_args)
        self.assertEqual([call('Syncing pg_data of dbid 2')],
                         self.mock_logger.debug.call_args_list)

    @patch('gppylib.commands.unix.Rsync.__init__', return_value=None)
    @patch('gppylib.commands.unix.Rsync.run')
    def test_sync_wals_and_control_file_success(self,run, init):
        self.diff_recovery_cmd.sync_wals_and_control_file()
        self.assertEqual(2, init.call_count)
    ...
    @patch('gppylib.commands.unix.Rsync.__init__', return_value=None)
    @patch('gppylib.commands.unix.Rsync.run')
    def test_sync_tablespaces_outside_data_dir(self,run, init, mock):
    ...
    @patch('gppylib.commands.unix.Rsync.__init__', return_value=None)
    @patch('gppylib.commands.unix.Rsync.run')
    def test_sync_tablespaces_within_data_dir(self, run, init, mock):
        self.diff_recovery_cmd.sync_tablespaces()
        self.assertEqual(0, init.call_count)
    ...
    @patch('gppylib.commands.unix.Rsync.__init__', return_value=None)
    @patch('gppylib.commands.unix.Rsync.run')
    def test_sync_tablespaces_mix_data_dir(self, run, init, mock):
        self.diff_recovery_cmd.sync_tablespaces()
        self.assertEqual(1, init.call_count)
        self.assertEqual(1, run.call_count)
        self.assertEqual(call(validateAfter=True), run.call_args)
    ...
    @patch('gpsegrecovery.DifferentialRecovery.pg_start_backup', side_effect=Exception())
    def test_diff_recovery_pg_start_backup_exception(self, mock1):
        self.diff_recovery_cmd.run()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed')],
                         self.mock_logger.debug.call_args_list)
    ...
    def test_diff_recovery_create_slot_exception(self, mock1,mock2):
        self.diff_recovery_cmd.run()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed')],
                         self.mock_logger.debug.call_args_list)
    ...
    def test_diff_recovery_sync_pg_data_exception(self, mock1, mock2):
        self.diff_recovery_cmd.run()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed')],
                         self.mock_logger.debug.call_args_list)
    ...
    @patch('gpsegrecovery.DifferentialRecovery.pg_start_backup', return_value=Mock())
    @patch('gpsegrecovery.DifferentialRecovery.sync_tablespaces', side_effect=Exception())
    def test_diff_recovery_sync_tablespaces_exception(self, mock1, mock2):
        self.diff_recovery_cmd.run()
        self.assertEqual(2, self.mock_logger.debug.call_count)
        self.assertEqual([call('Syncing pg_data of dbid 2'),
    ...
    @patch('gpsegrecovery.DifferentialRecovery.pg_stop_backup', side_effect=Exception())
    def test_diff_recovery_pg_stop_backup_exception(self, mock1, mock2, mock3,mock4):
        self.diff_recovery_cmd.run()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed')],
                         self.mock_logger.debug.call_args_list)
    ...
    def test_diff_recovery_write_conf_files_exception(self, mock1, mock2, mock3,mock4,mock5):
        self.diff_recovery_cmd.run()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed')],
                         self.mock_logger.debug.call_args_list)
    ...
    def test_diff_recovery_sync_wals_and_control_file_exception(self, mock1, mock2, mock3, mock4, mock5, mock6):
        self.diff_recovery_cmd.run()
        self.assertEqual(1, self.mock_logger.debug.call_count)
        self.assertEqual([call('Doing clean up as differential recovery has failed')],
                         self.mock_logger.debug.call_args_list)
    ...
    @patch('gppylib.commands.pg.PgBaseBackup.__init__', return_value=None)
    @patch('gppylib.commands.pg.PgBaseBackup.run')
    def test_diff_recovery_modify_conf_setting_exception(self,run,init,mock1,mock2,mock3,mock4,mock5):
        self.mock_pgbasebackup_init = init
        self.mock_pgbasebackup_run = run
        self.mock_pgbasebackup_modifyconfsetting.side_effect = [Exception('modify conf port failed'), Mock()]
        self.diff_recovery_cmd.run()
    ...
    @patch('gppylib.commands.pg.PgBaseBackup.__init__', return_value=None)
    @patch('gppylib.commands.pg.PgBaseBackup.run')
    def test_diff_recovery_start_segment_exception(self,run,init,mock1,mock2,mock3,mock4,mock5):
        self.mock_pgbasebackup_init = init
        self.mock_pgbasebackup_run = run
        gpsegrecovery.start_segment.side_effect = [Exception('pg_ctl start failed'), Mock()]
    ...
    @patch('gppylib.commands.pg.PgBaseBackup.__init__', return_value=None)
    @patch('gppylib.commands.pg.PgBaseBackup.run')
    def test_diff_recovery_basebackup_run_one_exception(self,run,init,mock1,mock2,mock3,mock4,mock5):
        self.mock_pgbasebackup_run = run
        self.mock_pgbasebackup_init = init
        self.mock_pgbasebackup_run.side_effect = [Exception('backup failed once'), Mock()]
    ...
    @patch('gppylib.commands.pg.PgBaseBackup.__init__', return_value=None)
    @patch('gppylib.commands.pg.PgBaseBackup.run')
    def test_diff_recovery_basebackup_init_exception(self,run,init,mock1,mock2,mock3,mock4,mock5):
        self.mock_pgbasebackup_run = run
        self.mock_pgbasebackup_init = init
        self.mock_pgbasebackup_init.side_effect = [Exception('backup init failed')]
